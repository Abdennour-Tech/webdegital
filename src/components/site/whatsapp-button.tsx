...
export function WhatsAppButton() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 rtl:right-auto rtl:left-5">
      {/* Tooltip */}
      <div
        aria-hidden="true"
      ...
      >
        <div className="whitespace-nowrap rounded-xl border border-border bg-background px-3 py-1.5 text-xs font-medium text-foreground shadow-card">
          {CONTACT.phoneDisplay}
          <span className="absolute -bottom-1 right-4 h-2 w-2 rotate-45 border-r border-b border-border bg-background rtl:right-auto rtl:left-4" />
        </div>
      </div>
      ...
      >
        {/* WhatsApp Icon Badge */}
        <span className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#25D366] text-white shadow-sm transition-transform duration-300 group-hover:rotate-6">
          <svg
            viewBox="0 0 24 24"
            width="18"
            height="18"
          ...
          <span className="absolute -top-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-emerald-400 border-2 border-primary rtl:-right-auto rtl:-left-0.5" />
        </span>
