...
async function getStartManifest(matchedRoutes) {
  var _a;
  const { tsrStartManifest } = await import("./_tanstack-start-manifest_v-DObzmQLc.js");
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import("./router-C6sJ9pzL.js").then((n) => n.r),
    import("./start-CjulQTtC.js"),
...
function disposeLateResponse(result, signal) {
  var _a;
  const response = (_a = handleCtxResult(result)) == null ? void 0 : _a.response;
...
async function executeMiddleware(middlewares, ctx, signal) {
  ...
  const disposeStreamResponse = async (reason) => {
    ...
    retiredStreamIdentities ?? (retiredStreamIdentities = /* @__PURE__ */ new WeakSet());
  ...
  const disposeAbandonedResult = (result) => {
    var _a;
    const exposed = (_a = handleCtxResult(result)) == null ? void 0 : _a.response;
...
function createStartHandler(cbOrOptions) {
  ...
  const startRequestResolver = async (request, requestOpts) => {
    ...
    try {
      ...
      const startOptions = await waitForRequest((_a = entries.startEntry.startInstance) == null ? void 0 : _a.getOptions(), request.signal) || {};
      ...
      const executeRouter = async (serverContext, matchedRoutes) => {
        ...
        attachRouterServerSsrUtils({
          ...
          getRequestAssets: () => {
            var _a3;
            return (_a3 = getStartContext({ throwIfNotFound: false })) == null ? void 0 : _a3.requestAssets;
          }
        });
        routerInstance.options.additionalContext = { serverContext };
        await routerInstance.load({ _signal: request.signal });
        request.signal.throwIfAborted();
        if (((_a2 = routerInstance._serverResult) == null ? void 0 : _a2.type) === "redirect") return normalizeSsrResponse(routerInstance._serverResult.redirect);
        earlyHints == null ? void 0 : earlyHints.collectDynamic(_getRenderedMatches(routerInstance.stores.matches.get()));
        const ctx = getStartContext({ throwIfNotFound: false });
        await waitForRequest(routerInstance.serverSsr.dehydrate({ requestAssets: ctx == null ? void 0 : ctx.requestAssets }), request.signal);
        request.signal.throwIfAborted();
